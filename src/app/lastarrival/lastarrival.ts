...
export class Lastarrival implements OnInit{
  ...
  getFilterOrdres(){
    return this.totfiltres.filter((o:any)=>!o.selected).sort((a:any,b:any)=>{
        return String(a[this.lg]).localeCompare(String(b[this.lg]));
    })
  }
  ...
  deleteFilters(){
    this.totfiltres.forEach((t:any)=>{t.excluded=[];t.selected = false});
  }
  ...
  getTri(montre:any){
    if(this.tri=="marque"||this.tri=="reference"||this.tri=="movement") return;
    return this.data.ordre.find((o:any)=>o.id==this.tri)[this.lg] + "<br/>" + montre[this.lg][this.tri];
  }
  ...
  nextPage(nb:number) {
    if(nb==-1){
      ...
      if(this.scroll<this.montres.length-1)
        this.scroll = this.scroll + 1;
    }
  ...
  getMontres(i:any)
  {
    ...
    this.montresActuelles = this.montres.filter((m:any)=>m[this.lg][this.filter] == this.menus[this.filterClicked]);
  }
  ...
  getMontresActuelles(){
    ...
    if(this.catalog)
    {
      this.totfiltres.filter((f:any)=>f.selected).forEach((f:any)=>{
